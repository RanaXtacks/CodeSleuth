...
export default function FindingsPanel({ results, loading }) {
  ...
  if (loading) {
    return (
      <div className="glass-panel p-8 rounded-2xl shadow-xl border border-slate-800/80 h-full flex flex-col items-center justify-center text-center space-y-4">
  ...
  if (!parsedData) {
    return (
      <div className="glass-panel p-8 rounded-2xl shadow-xl border border-slate-800/80 h-full flex flex-col items-center justify-center text-center space-y-4">
        ...
        <div>
          <h3 className="text-base font-bold text-white tracking-tight">Awaiting Code Submission</h3>
  ...
  return (
    <div className="glass-panel p-6 rounded-2xl shadow-2xl border border-slate-800/80 h-full overflow-y-auto space-y-6">
      {/* Overview Header */}
      <div className="flex items-center justify-between pb-4 border-b border-slate-800/80">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Static analysis grounded by Semgrep &amp; Gemini AI</p>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            {securityFindings.map((item, i) => (
              <div key={i} className="p-4 bg-slate-900/80 rounded-xl border border-rose-900/30 space-y-2.5">
                ...
                {item.suggested_fix && (
                  <div className="pt-2 border-t border-slate-800/60">
                    <span className="text-[10px] uppercase font-semibold text-emerald-400 tracking-wider">Suggested Fix:</span>
                    <pre className="mt-1 p-3 bg-slate-950 rounded-lg text-xs font-mono text-emerald-300 border border-emerald-900/30 overflow-x-auto">
                      {item.suggested_fix}
                    </pre>
                  </div>
                )}
      ...
      {bugs.length > 0 && (
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            {bugs.map((item, i) => (
              <div key={i} className="p-4 bg-slate-900/80 rounded-xl border border-amber-900/30 space-y-2.5">
                <div className="flex items-center justify-between">
                  <span className="bg-slate-950 px-2 py-0.5 rounded border border-slate-800 text-amber-300 font-mono text-xs">
                    {item.file || 'source.py'}{item.line ? `:${item.line}` : ''}
                  </span>
                  <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border ${getSeverityBadge(item.severity)}`}>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            {performanceNotes.map((item, i) => (
              <div key={i} className="p-4 bg-slate-900/80 rounded-xl border border-cyan-900/30 space-y-2">
                <p className="text-xs text-slate-200 font-sans">{item.description}</p>
                {item.suggestion && (
                  <p className="text-xs text-cyan-300 font-mono bg-cyan-950/40 p-2.5 rounded-lg border border-cyan-800/30">
                    💡 {item.suggestion}
                  </p>
                )}
              </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs font-semibold text-slate-200 flex items-center">
                      <span className="w-1.5 h-1.5 rounded-full mr-2 bg-indigo-400" />
                  ...
                  <div>
                    ...
                    <pre className="mt-1 p-3 bg-slate-950 rounded-lg text-[11px] font-mono text-slate-300 border border-slate-800 overflow-x-auto max-h-48 leading-relaxed">
                      {test.generated_code}
                    </pre>
                  </div>
                  ...
                  {test.execution?.stderr && (
                    <div>
                      ...
                      <pre className="mt-1 p-3 bg-slate-950 border border-rose-900/40 rounded-lg text-[11px] font-mono text-rose-300 overflow-x-auto max-h-36 whitespace-pre-wrap">
                        {test.execution.stderr}
                      </pre>
                    </div>
                  )}
